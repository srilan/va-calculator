...
export default function App() {
  ...
  const [childrenUnder18, setChildrenUnder18] = createSignal(0);
  const [childrenAbove18, setChildrenAbove18] = createSignal(0);
  const [hasSpouse, setMaritalStatus] = createSignal(false);
  const [aidAndAttendance, setAidAndAttendance] = createSignal(false);
  const [dependentParents, setDependentParents] = createSignal(0);
  const [monthlyPayment, setMonthlyPayment] = createSignal(0);
  const [ratingId, setRatingId] = createSignal(1);

  const [disabilityRating, setDisabilityRating] = createSignal({
    bilateralFactor: 0,
    calculatedRating: 0,
    disabilityRating: 0,
  });

  const [monthly, setMonthly] = createSignal({
    monthly: 0,
  });
  ...
  const [disabilityLoading, setDisabilityLoading] = createSignal(false);
